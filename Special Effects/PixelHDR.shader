	...
	SubShader
	{
		...
		Pass
		{
				...
				) : SV_Target
			{
				...
				float2 uv_Pixel = i.uv0XY_bitZ_fog.xy * _Pixel_ST.xy + _Pixel_ST.zw;
				float4 EmissionMap540 = ( tex2D( _Pixel, uv_Pixel ) * _PixelColor * tex2D( _EmissionMap, UV_Main492 ) );
		...
		Pass
		{
			...
			half4 frag(v2f i  ) : SV_Target
			{
				...
				float2 uv_Pixel = i.uv * _Pixel_ST.xy + _Pixel_ST.zw;
				float4 EmissionMap540 = ( tex2D( _Pixel, uv_Pixel ) * _PixelColor * tex2D( _EmissionMap, UV_Main492 ) );
				float4 EmissionColor540 = _EmissionColor;
				float4 Colorshiftbool514 = staticSwitch293;
				float4 Albedo540 = Colorshiftbool514;
